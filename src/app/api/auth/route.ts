...
function getErrorDetails(error: unknown): { message: string; code?: string } {
  if (error instanceof Error) {
    ...
    // Common Prisma/DB errors that users can fix
    if (msg.includes('Can\'t reach database server') || msg.includes('connect ETIMEDOUT') || msg.includes('ECONNREFUSED')) {
      return { message: 'Database connection failed. Please check DATABASE_URL in Vercel environment variables.', code: 'DB_CONNECTION' }
    }
    if (msg.includes('relation') && msg.includes('does not exist')) {
      return { message: 'Database tables not found. Run "prisma db push" against your PostgreSQL database.', code: 'DB_MIGRATION' }
    }
    ...
    if (msg.includes('authentication failed') || msg.includes('password authentication')) {
      return { message: 'Database authentication failed. Check DATABASE_URL credentials.', code: 'DB_AUTH' }
    }
    if (msg.includes('too many connections') || msg.includes('connection_limit')) {
      return { message: 'Database connection limit reached. Use a pooled connection URL (Neon: add -pooler to hostname).', code: 'DB_POOL' }
    }
